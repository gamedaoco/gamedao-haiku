...
import type { relativeTimeRounding } from 'moment'
...
export const validation = Yup.object({
	...
	legal: Yup.string().max(32, 'Max of 32 characters').notRequired(),
...
export function useIdentitySetTransaction(identity): TransactionData {
	...
	useEffect(() => {
		if (!selectedApiProvider?.apiProvider || !identity) return
