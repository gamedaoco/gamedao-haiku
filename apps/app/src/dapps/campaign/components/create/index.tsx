...
import { Content, validationSchema as contentValidationSchema } from './Content'
import { Name, validationSchema as nameValidationSchema } from './Name'
import { Settings, getValidationSchema as getSettingsValidationSchema } from './Settings'
...
export function CreateCampaign({ collectiveId, cancel, currentStep, setStep, draftId }: ComponentProps) {
	...
	const { addDraft, drafts } = useSaveCampaignDraft(collectiveId)
	const { t } = useTranslation()

	useEffect(() => {
		tmpCampaignState.setOrgId(collectiveId)
	}, [tmpCampaignState, collectiveId])

	useEffect(() => {
		if (draftId && drafts[draftId]) {
			tmpCampaignState.restoreDraft({ ...drafts[draftId], orgId: collectiveId })
		}
	}, [draftId, drafts])
	...
	return (
		<>
			...
			<Stack spacing={2} sx={{ justifyContent: { xs: 'space-between', sm: 'flex-start' } }} direction="row">
				...
				{draftId && (
					<Button size="large" variant="outlined" color="error" onClick={() => setOpenConfirmDeleteModal(true)} sx={{ flexGrow: { xs: 1, sm: 0 } }}>
						{t('button:ui:delete_draft')}
					</Button>
				)}

				{currentStep > 0 && (
					<Button size="large" variant="outlined" onClick={handleSaveDraft} sx={{ flexGrow: { xs: 1, sm: 0 } }}>
			...
			<TransactionDialog open={txModalState} onClose={handleCloseTxModal} txData={createCampaignTx} txCallback={handleTxCallback} />
			<ConfirmDeleteCampaignDraft orgId={collectiveId} id={draftId} onClose={handleConfirmDeleteModalOnClose} open={openConfirmDeleteModal} />
		</>
	)
}
