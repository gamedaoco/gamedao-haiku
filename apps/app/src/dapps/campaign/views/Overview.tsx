...
export function CampaignDApp() {
	...
	if (campaignsCount?.data?.campaign_aggregate?.aggregate.count === 0) return <Typography fontWeight={700}>No Campaigns yet — why not create one!</Typography>
		...
		<Fragment>
			{paginatedData?.length === 0 && !loading && (
				<Box sx={{ mt: 2, mb: 4 }}>
					...
					{filters.query && <Typography>{t('page:campaigns:no_result', { query: filters.query })}</Typography>}
				</Box>
			)}
			...
			<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
				<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', my: 3 }}>
					...
					>
						{buttonVisibility && (
							<Button endIcon={<ArrowDownward />} onClick={() => setLimit((p) => p + 30)} variant="outlined">
