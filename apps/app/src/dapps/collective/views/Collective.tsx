...
const GovernanceView = dynamic(() => import('../components/tabs/governance/overview').then((mod) => mod.GovernanceView))
const ProposalView = dynamic(() => import('../components/tabs/governance/proposal').then((mod) => mod.ProposalView))
...
export const Unity = ({ args }: TProps) => {
	const { view, organization, id, isAdmin } = args
	return (
		<Fragment>
			<Stack spacing={4}>
				<Paper
						...
						<Container disableGutters maxWidth="xl" sx={{ border: 0 }}>
							<TabContext value={view.toString()}>
								<TabPanel value="dashboard">
									<DashboardView args={args} />
								</TabPanel>
								<TabPanel value="campaigns">
									<CampaignsView organizationId={id} isAdmin={isAdmin} />
								</TabPanel>
								{/*
								 */}
								<TabPanel value="treasury">
									<TreasuryView args={args} />
								</TabPanel>
								<TabPanel value="proposal">
									<ProposalView args={args} />
								</TabPanel>
								<TabPanel value="governance">
									<GovernanceView args={args} />
								</TabPanel>
								<TabPanel value="members">
									<MembersView organizationState={organization} />
								</TabPanel>
								<TabPanel value="settings">
									<SettingsView args={args} />
								</TabPanel>
								{/*
								 */}
							</TabContext>
