...
export function DashboardView(args: TArgs) {
	const { organization, id, isMember, isAdmin, handleOpenTxModal, handleCloseTxModal, showTxModalType, addMemberTx } =
		args
		...
		() =>
			organization?.member_limit === 0
				...
				: {
						...
						text: ` ${t('page:organizations:organization_rules:memberLimit:only')} ${organization?.member_limit}  ${t(
							'page:organizations:organization_rules:memberLimit:text',
						)}`,
					},
									...
									<Stack direction="row" spacing={1} color={theme.palette.text.secondary}>
										...
										<Box>
											{orgData.tags.map((item, index) => {
												return (
													<Chip size="small" sx={{ mr: 1, mb: 1 }} key={index} label={item} />
												)
											})}
										</Box>
									</Stack>
