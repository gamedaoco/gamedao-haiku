...
export function Header({ args }: TProps) {
	...
	const Background = ({ id, name, imageUrl, handleUploadImage }) => {
		return (
			<Grid
			...
			>
				<label htmlFor="header-file-upload">
					<input
						...
						onClick={isPrime ? (event) => handleUploadImage(event, cache.setHeaderCID) : null}
					/>
					{imageUrl ? (
						...
						<Box display="grid" justifyContent="center" alignItems="center" sx={{ width: '100%', height: '100%' }}>
	...
	return organization ? (
		...
		>
			...
			>
				<label htmlFor="logo-file-upload">
					<input
						...
						type="file"
						// disabled={!!organizationIdState}
						onChange={isPrime ? (event) => handleUploadImage(event, cache.setLogoCID) : null}
					/>
					<Avatar
						sx={(theme) => ({
							width: '5rem',
			...
			*/}
			...
			<Stack minHeight={200} spacing={2} justifyContent="space-between">
				<Background
					id={organization.id}
					name={organization.name}
					imageUrl={parseIpfsHash(organization.header, config.IPFS_GATEWAY)}
