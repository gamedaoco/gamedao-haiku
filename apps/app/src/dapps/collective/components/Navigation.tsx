...
export function Navigation({ args }: TProps) {
		...
		() =>
			[
				true
					? {
							label: 'Dashboard',
							value: CollectiveView.DASHBOARD,
							disabled: false,
						}
					: null,
				true
					? {
							label: 'Governance',
							value: CollectiveView.GOVERNANCE,
							disabled: false,
						}
					: null,
				true
					? {
							label: 'Fundraising',
							value: CollectiveView.CAMPAIGNS,
							disabled: true,
						}
					: null,
				true
	...
	const handleTabChange = useCallback(
		(event: ChangeEvent<{}>, view: CollectiveView): void => {
			console.log('tab change', view)
			push(`/collectives/${id}/${view}`)
		},
		[id, push],
	)
