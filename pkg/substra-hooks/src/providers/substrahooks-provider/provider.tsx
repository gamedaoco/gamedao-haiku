...
const initAllApis = async (apiProviderConfig: ApiProviderConfig) => {
	return Promise.all(
		Object.keys(apiProviderConfig).map(async (configId) =>
			initPolkadotPromise(apiProviderConfig[configId].id, apiProviderConfig[configId].wsProviderUrl),
		),
	)
}
	...
	const SubstraHooksProvider = ({
		...
		return (
			<SubstraHooksContext.Provider value={{ apiProviders, defaultApiProviderId }}>
				<ExtensionProvider autoInitialiseExtension={autoInitialiseExtension}>{children}</ExtensionProvider>
